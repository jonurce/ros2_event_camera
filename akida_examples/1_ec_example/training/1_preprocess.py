...
# ========================================
T_BINS = 10
WINDOW_US = T_BINS * 10000  # Labeled at 100Hz = 0.01s = 10000us
H, W = 480, 640
...
def events_to_voxel_gpu(events, t_start, t_end):
    ...
    t_norm = torch.clamp(t_norm, 0, T_BINS - 1e-6)
    bin_idx = t_norm.floor().long()

    # Polarity
    pol = torch.where(p == 1, 1.0, -1.0).to(DEVICE)

    # Voxel
    voxel = torch.zeros((T_BINS, H, W), device=DEVICE, dtype=torch.float32)
    flat_idx = bin_idx * (H * W) + y * W + x
    flat_voxel = voxel.view(T_BINS, -1)

    # Scatter add
    flat_voxel.scatter_add_(1, flat_idx.unsqueeze(0).expand(T_BINS, -1), pol.unsqueeze(0))

    return flat_voxel.view(T_BINS, W, H).cpu()
...
if __name__ == "__main__":
    ...
    print(f"Format: .pt (10,640,480) + .label.txt")
    ...
    print("Files created:")
    print(" preprocessed/train/REC_ID/sample_XXX.pt")
    print(" preprocessed/train/REC_ID/sample_XXX.label.txt")
