...
class FlattenTimeWrapper(torch.nn.Module):
    ...
    def forward(self, input):
        # Input: [B, 100, 96, 128]
        b, _, h, w = input.shape
        input = input.view(b, 50, 2, h, w)          # → [B, T, C, H, W]
        input = input.permute(0, 2, 1, 3, 4)        # → [B, C, T, H, W] = [B, 2, 50, 96, 128]

        output = self.base_model(input)         # → [B, 3, 50, 3, 4]
        return output[:, :, -1, :, :]       # → [B, 3, 3, 4] (last time step only)
...
for i, batch in enumerate(tqdm(calib_loader, total=25)):
    ...
    # x = batch['input'].float().div(255.0).numpy()   # ← float32 + normalized
